...
#
FAILURE_ID_GROPS = ("fid", "ip4", "ip6", "dns", "host")
...
class FailRegex(Regex):
	...
	def getIPS(self):
		if "ips" in self._regexObj.groupindex:
			ips = self._matchCache.group("ips")
			return str(ips)
		return None

	def getDOMAIN(self):
		if "domain" in self._regexObj.groupindex:
			domain = self._matchCache.group("domain")
			return str(domain)		
		return None
