...
class Ticket(object):
	...
	def __init__(self, ip=None, time=None, matches=None, ips=None, data={}, ticket=None):
		"""Ticket constructor
	...
	def getIPS(self):
		if self.__ips:
			return list(self.__ips)[0]
		else:
			return None

	def addIPS(self, value):
		self.__ips.add(value)

	def containIPS(self, value):
		return value in self.__ips
...
class FailTicket(Ticket):
	...
	def __init__(self, ip=None, time=None, matches=None, data={}, ticket=None, ips=None):
		# this class variables:
		self._firstTime = None
		self._retry = 1
		# create/copy using default ticket constructor:
		Ticket.__init__(self, ip, time, matches, ips, data, ticket)
		# init:
	...
	def adjustTime(self, time, maxTime):
		...
		if time > self._time:
			...
			if self._firstTime < time - maxTime:
				...
				logSys.info("TW retry: %s, time : %s, firstTime : %s, maxTime: %s", self._retry, time, self._firstTime, maxTime)
